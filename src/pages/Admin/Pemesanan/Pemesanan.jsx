    ...
    <div className="bg-[#EDF0F2] flex flex-col min-h-[calc(100vh-92px)] pt-[4px] pl-[13px] pr-[8px] pb-[8px] font-sans text-gray-800">
      ...
      <div className="bg-[#F5F5F5] flex-1 grid grid-cols-[1fr_198px] overflow-hidden">
        <div className="relative shadow-[0px_2px_6px_rgba(156,156,156,0.25)] border border-[#959595] h-full">
          ...
          <table className="table-auto w-full [&_th]:border-[1px] [&_th]:border-r">
            ...
            {data.length !== 0 ? data.map((val, ind) => (
                <tr
                  key={ind}
                  className="even:bg-[#DCDCDC] [&_td]:h-[33px] [&_td]:border-r [&_td]:border-[#959595] text-[14px] font-semibold"
                >
                  <td className="pl-[21px] ">{val.nama}</td>
                  <td>
                    <div className="text-center flex justify-around items-center h-full">
                      <button onClick={()=>{handleDecrement(ind)}} className="cursor-pointer h-full flex-1 flex justify-center items-center">
                        <img src={MinRed} alt="" />
                      </button>
                      <input
                        type="text"
                      ...
                      />
                      <button onClick={()=>handleIncrement(ind)} className="cursor-pointer h-full flex-1 flex justify-center items-center">
                        <img src={PlusGreen} alt="" />
                      </button>
                    </div>
                  </td>
                  <td className="text-center">
                    Rp {Number(val.harga).toLocaleString("id-ID")}
                  </td>
                  <td>
        ...
        <div className="flex flex-col justify-between mx-[14px]">
          <button onClick={(()=>setIsOpen(true))} className="bg-[#44962D] hover:bg-[#3E8C29] active:bg-[#3A7D27] h-[43px] rounded-[10px] flex gap-[7.94px] items-center justify-center cursor-pointer">
            <img src={Plus} alt="plus" />
            <p className="text-[14px] font-bold text-white">Tambah Pesanan</p>
          </button>
          <div className="space-y-[8px] font-semibold">
            <p>Metode Pembayaran</p>
            <button onClick={() => choosePayment("cash")} className={`${paymentMethod === "cash" ? "bg-[#E2E2E2] text-[#5D5D5D]" : "bg-white" } transition-colors flex w-full items-center cursor-pointer px-[13px] gap-[20px] h-[50px] text-[20px]`}>
                <img src={Cash} alt="" className="" />
              <p>Cash</p>
            </button>
            <button onClick={() => choosePayment("ewallet")} className={`${paymentMethod === "ewallet" ? "bg-[#E2E2E2] text-[#5D5D5D]" : "bg-white" } transition-colors flex w-full items-center cursor-pointer px-[13px] gap-[20px] h-[50px] text-[20px]`}>
      ...
      <div className="mt-[7px] h-[105px] bg-white shadow-[0px_2px_6px_rgba(156,156,156,0.25)] p-[10px] flex justify-end items-center">
        <div className="font-bold space-y-1 border-[1.4px] border-[#959595] h-full flex w-[309px] text-[14px] items-center justify-around">
            ...
            <p className="text-xl text-[#FFB300]">
              Rp. {Number(200000).toLocaleString("id-ID")}
            </p>
        </div>
      ...
      <TambahPesanan isOpen={isOpen} setIsOpen={setIsOpen} setData={setData} data={data} editIndex={editIndex} setEditIndex={setEditIndex}/>
    </div>
  );
}
